...
BOOSTER_DICT = \
    {"absolutely": B_INCR, "amazingly": B_INCR, "awfully": B_INCR,
     ...
     "occasional": B_DECR, "occasionally": B_DECR, "only":B_DECR, "partly": B_DECR,
...
def ngram_convert(text, defined_bigrams, defined_trigrams):
      text_stripped = text.strip(string.punctuation)
      ...
      trigrams = list(nltk.trigrams(tokens)) # create bigrams as tuples of tokens
      trigrams = list(map(''.join, trigrams)) # join each word without space to create new bigram
      trigrams = trigrams + ['...', '...'] # make length of tokens and bigrams list equal
        
      bigrams_df = pd.read_csv(defined_bigrams, sep = ';')
      trigrams_df = pd.read_csv(defined_trigrams, sep = ';')
      
      bigrams_dic = dict(zip(bigrams_df.bigram, bigrams_df.score))
      trigrams_dic = dict(zip(trigrams_df.trigram, trigrams_df.score))
      ...
      for i, token in enumerate(tokens):
          ...
          if b.lower() in bigrams_dic:
            join_word = b # replace the word in text by bigram
            tokens[i+1] = '' #skip the next word
          elif t.lower() in trigrams_dic:
            join_word = t # replace the word in text by bigram
            tokens[i+1] = ''  
            tokens[i+2] = '' 
          else:
              join_word = token
          final_text += join_word + ' '
...
class SentimentIntensity(object):
    """
    Give a sentiment intensity score to sentences.
    """

    def __init__(self, lexicon_file="ModVad_lexicon.txt", emoji_lexicon="ModVad_emoji_lexicon.txt"):
    ...
    def polarity_scores(self, text):
        ...
        text = re.sub(r"t's","ts",text)
        
        final_text = ngram_convert(text, 'bigram_change.csv', 'trigram_change.csv')
     
        sentitext = SentiText(final_text)
    ...
    def score_valence(self, sentiments, text):
        if sentiments:
            ...
            if sum_s > 0:
                ...
                sum_s -= ep_amplifier
            else: sum_s += ep_amplifier * 5
                
            sum_s = sum_s * qm_amplifier
            ...
            pos_sum = pos_sum * qm_amplifier
            neg_sum = neg_sum * qm_amplifier
